...
function doPost(e) {
  ...
  if (action === "listPreFinal") return jsonResponse_({ ok: true, reports: listPreFinalReports_() });
  if (action === "savePreFinal") return savePreFinalReports_(body.reports);
...
// Inspection backend — one JSON blob per report row. Every save also
// rebuilds the Entries rows for stages "prefinal"/"final" so Style
// Tracking's rollup reflects the current set of reports exactly.
const PREFINAL_SHEET = "PreFinalReports";

function listPreFinalReports_() {
  const sheet = getSheet_(PREFINAL_SHEET, ["id", "json"]);
  const rows = sheet.getDataRange().getValues();
  const out = [];
  for (let i = 1; i < rows.length; i++) {
...
function savePreFinalReports_(reports) {
  reports = reports || [];
  const sheet = getSheet_(PREFINAL_SHEET, ["id", "json"]);
  ...
  resyncPreFinalEntries_(reports);
  return jsonResponse_({ ok: true, count: reports.length });
}

function resyncPreFinalEntries_(reports) {
  const sheet = getSheet_(ENTRIES_SHEET, ENTRIES_HEADERS);
  const data = sheet.getDataRange().getValues();
  const keep = [ENTRIES_HEADERS];
  for (let i = 1; i < data.length; i++) {
    const stage = data[i][1];
    if (stage === "prefinal" || stage === "final") continue;
    keep.push(data[i]);
  }
  reports.forEach(function (r) {
    const h = r.header || {};
    const stage = h.inspectionType === "Final" ? "final" : "prefinal";
    keep.push([
      h.date ? new Date(h.date) : new Date(),
      stage, "", h.styleNo || "", h.ir || "",
      h.inspectorName || "",
      r.result === "PASS" ? "Done" : "Open",
      r.result || "",
      r.remarks || ""
    ]);
  });
