...
const RA_HEADERS = ["SBU","Buyer","IR","StyleName","Item","ProductDept","Season","TotalQty","ComplexityJSON","OverallComplexity","Notes","CreatedAt"];
...
function saveRA_(rec) {
  ...
  sheet.appendRow([
    ...
    JSON.stringify(rec.complexity || {}), rec.overallComplexity || "", rec.notes || "",
    new Date()
  ]);
...
function listRA_() {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    out.push({
      ...
      season: r[6], totalQty: r[7], complexity: complexity, overallComplexity: r[9],
      notes: r[10], createdAt: r[11]
    });
  }
  return out;
}
